...
import requestMapResolver from './request-map-resolver'
...
export const handler = (event, context, callback) => {
  ;(async () => {
    try {
      ...
      const { url, type, authorizationToken, methodArn } = event
      const user = await signin(type, authorizationToken)
      if (!user) return callback('Unauthorized')

      const isResourceAllowed = await requestMapResolver(user, url)
      if (!isResourceAllowed) return callback('Unauthorized')
